...
import { IGAccountViewProps } from "../types"

const IGAccountView = ({ results, loading }: IGAccountViewProps) => {
  ...
  useEffect(() => {
    const fetchRecommendation = async () => {
      ...
      try {
        const clientRes = await fetch("http://127.0.0.1:8000/client-info")
        ...
        const res = await fetch("http://127.0.0.1:8000/socials-recommendation", {
          ...
          body: JSON.stringify({
            posts: results.data.map((post) => post.caption || ""),
            client: clientInfo,
          }),
        })
      ...
      <div className="grid col-span-1 gap-6 p-3 post-area border-2 border-green-300 rounded-xl">
        {loading
          ...
          : results.data.map((post, index) => (
              <Post key={post.id} post={post} index={index} content_type="ig_acc" />
            ))}
      </div>
